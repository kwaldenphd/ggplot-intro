...
We’ve seen `qplot` a few times by this point and will not cover it in this lab.

`ggplot()`, the grammar of graphics plot, is different from other graphics functions because it uses a particular grammar inspired by Leland Wilkinson’s landmark book, *The Grammar of Graphics*, which focuses on thinking about, reasoning with and communicating with graphics. It enables layering of independent components to create custom graphics.
...
For example, the following code will produce identical results: 
...
The `histogram` `geom` (`geom_histogram`) transforms the SalePrice, modifies the bin size and changes the color.
...
The `labs()` command adds a title and an x-axis label. 

A y-axis label can also be added by using `y = " `.
...
In these examples, we have created separate scatterplots based upon the number of fireplaces.

When assigning fixed characteristics, (such as color, shape or size), the commands occur outside the `aes`, as in `color="green"`. 

When characteristics are dependent on the data, the command should occur within the `aes`, as in `color=Kitchen.Qual`.

In the above examples, only a few `geoms` are listed. 

The [`ggplot2` website](https://ggplot2.tidyverse.org/reference/) lists each `geom` and gives detailed examples of how they are used.
...
If you use the `str` command after reading data into `R`, you will notice that each variable is assigned one of the following types: 
- `Character`
...
Notice that the `color` and `fill` commands now work properly.
...
In the above boxplot, `position_dodge(width = 1)` adds a space between each box. 

For scatterplots, `position = position_jitter()` puts spaces between overlapping points.
...
You can also install the [`ggthemes` package](https://yutannihilation.github.io/allYourFigureAreBelongToUs/ggthemes/) for many more options.
